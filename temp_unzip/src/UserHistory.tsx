...
export default function UserHistory({ history, setActiveTab }: any) {
          ...
          <tbody className="text-[13px]">
            ...
            {filteredHistory.map((item: any) => (
              <tr key={item.id} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors">
                <td className="py-4">
                  <span className={`px-2 py-1 rounded border text-[10px] font-bold uppercase ${item.type === "earn" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-yellow-500/10 text-yellow-500 border-yellow-500/20"}`}>
                    {item.type}
                  </span>
                </td>
                ...
                <td className="py-4 font-bold whitespace-nowrap">
                  <span className={item.type === 'earn' ? 'text-emerald-400' : 'text-white'}>
