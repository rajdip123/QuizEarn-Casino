...
export default function DailyBonus({ balance, setBalance, setNotification, setHistory, setActiveTab, currency, rate }: any) {
  ...
  const claimBonus = () => {
    ...
    setNotification(`🎉 Daily Bonus Claimed! You got ${currency === 'USDT' ? '$' : currency === 'BDT' ? '৳' : '₹'}{(rewardAmount * (rate||1)).toFixed(2)}`);
  };
            ...
            return (
              ...
              >
                ...
                {isCompleted ? (
                  ...
                  <div className="text-[11px] font-bold">{currency === 'USDT' ? '$' : currency === 'BDT' ? '৳' : '₹'}{(getRewardForDay(day) * (rate||1)).toFixed(2)}</div>
                )}
