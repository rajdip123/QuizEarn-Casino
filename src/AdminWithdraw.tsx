...
export default function AdminWithdraw({
  ...
  setHistory,
  setNotification,
  setNotificationsList, currency, rate
}: any) {
  const getCurrencySymbol = (cur: string) => cur === 'USDT' ? '$' : cur === 'BDT' ? '৳' : '₹';
  ...
  const updateWithdrawStatus = (id: number, status: string, screenshot: string | null) => {
    ...
    if (updatedW) {
      ...
      setNotificationsList((prev: any) => [{
        ...
        message: `Your withdraw request of ${getCurrencySymbol(updatedW.currency || 'INR')}${(updatedW.amount * (rate||1)).toFixed(2)} was ${status.toLowerCase()}.`,
          ...
          <div key={item.id} className="bg-[#050505]/80 backdrop-blur-md border border-yellow-500/5 shadow-inner rounded-xl p-4 relative">
            <div className="grid grid-cols-2 gap-4 mb-3">
              <div>
                ...
                <div className="font-bold text-white">{getCurrencySymbol(item.currency || 'INR')}{(item.amount * (rate||1)).toFixed(2)}</div>
              </div>
              <div>
                <div className="text-[10px] text-white/40 mb-1 uppercase">Final</div>
                <div className="font-bold text-emerald-400">{getCurrencySymbol(item.currency || 'INR')}{(item.finalAmount * (rate||1)).toFixed(2)}</div>
              </div>
              <div>
