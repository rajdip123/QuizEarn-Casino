...
import { Target, Dice5, Tv, Gift, Users, Brain, CircleDashed, Dices, Coins } from 'lucide-react';

export default function GamesTab({ setActiveTab, currency, rate }: any) {
  return (
    <div className="space-y-4 pb-20">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div onClick={() => setActiveTab('quiz_categories')} className="bg-[#0a0a0a] border border-yellow-500/30 rounded-xl p-4 cursor-pointer hover:bg-[#1a1c24] transition-colors relative group">
          ...
          <div className="font-bold text-white text-[15px]">Casino Quiz</div>
        ...
        <div onClick={() => setActiveTab('wheel')} className="bg-[#0a0a0a] border border-amber-500/30 rounded-xl p-4 cursor-pointer hover:bg-[#1a1c24] transition-colors relative group">
          ...
          <CircleDashed className="w-8 h-8 text-amber-400 mb-3" />
          <div className="font-bold text-white text-[15px]">Lucky Wheel</div>
          <div className="text-[11px] text-white/50 mt-1">Spin & Win up to {currency === "USDT" ? "$" : currency === "BDT" ? "৳" : "₹"}{(100 * (rate||1)).toFixed(2)}</div>
        </div>
        ...
        <div onClick={() => setActiveTab('slot')} className="bg-[#0a0a0a] border border-emerald-500/30 rounded-xl p-4 cursor-pointer hover:bg-[#1a1c24] transition-colors relative group col-span-2 sm:col-span-1">
          ...
          <Dices className="w-8 h-8 text-emerald-400 mb-3" />
          <div className="font-bold text-white text-[15px]">Royal Slots</div>
          <div className="text-[11px] text-white/50 mt-1">Bet & Multiply your cash</div>
        </div>

        {/* Coin Flip */}
        <div onClick={() => setActiveTab('coin_flip')} className="bg-[#0a0a0a] border border-yellow-500/30 rounded-xl p-4 cursor-pointer hover:bg-[#1a1c24] transition-colors relative group">
          <div className="absolute inset-0 bg-gradient-to-br from-yellow-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity rounded-xl" />
          <Coins className="w-8 h-8 text-yellow-400 mb-3" />
          <div className="font-bold text-white text-[15px]">Coin Flip</div>
          <div className="text-[11px] text-white/50 mt-1">Heads or Tails? Double it!</div>
        </div>
