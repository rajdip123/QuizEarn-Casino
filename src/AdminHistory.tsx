...
export default function AdminHistory({ history, withdrawals, currency, rate }: any) {
          ...
          <tbody className="text-[12px]">
            {filteredLogs.map((item: any, idx: number) => (
              <tr key={idx} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02]">
                ...
                <td className="py-3 font-bold">{getCurrencySymbol(currency || 'INR')}{(Number(item.amount) * (rate||1)).toFixed(2)}</td>
                <td className="py-3">
