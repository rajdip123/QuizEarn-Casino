...
export default function WatchAds({
  ...
  const maxAds = 10;
  const rewardAmount = adminSettings?.adReward || 0.5;
  ...
  const watchAd = () => {
    ...
    setTimeout(() => {
      ...
      setNotification(`🎉 You earned ${currency === 'USDT' ? '$' : currency === 'BDT' ? '৳' : '₹'}{(rewardAmount * (rate||1)).toFixed(2)}!`);
    }, 5000);
  };
  ...
  return (
    <div className="bg-[#0a0a0a] border border-blue-500/30 rounded-[20px] p-6 text-center shadow-[0_0_30px_rgba(59,130,246,0.1)]">
      ...
      <p className="text-[12px] text-white/50 mb-8">Earn {currency === 'USDT' ? '$' : currency === 'BDT' ? '৳' : '₹'}{(rewardAmount * (rate||1)).toFixed(2)} for every video you watch</p>
